...
def train_semi_finished_SiamAttenUNet(pt_path: str = None, out_path: str = None):
    if pt_path is None:
    ...
    trainer.report_loss = lambda loss, epoch: QQ.send_myself_QQEmail(
        "SiamAttenUNet Loss Report", "epoch: {}, loss: {}".format(epoch, loss))
...
def evaluate_SiamAttenUNet(pt_path: str = None, save_predict_img: bool = False, img_path: str = None):
    if pt_path is None:
        raise ValueError(pt_path)
